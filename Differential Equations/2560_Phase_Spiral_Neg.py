...
class phase_spiral_neg(Scene):
    def construct(self):
        ...
        v1_0_dir1 = A @ v1_0
        dir1_label1 = MathTex(
            r"\left. \frac{dx}{dt} \right|_{t=0} = "
        ).next_to(v1_0_label, DOWN, buff=0.5).align_to(v1_0_label, LEFT)
        dir1_label2p1 = MathTex(
            r"A \vec{x}(0)"
        ).next_to(dir1_label1, RIGHT)
        dir1_label2p2 = MathTex(
            r"A \begin{bmatrix} 2 \\ 2 \end{bmatrix}"
        ).next_to(dir1_label1, RIGHT)
        dir1_label2p3 = Matrix([
            [f"{v1_0_dir1[0]:.2f}"],
            [f"{v1_0_dir1[1]:.2f}"]
        ]).next_to(dir1_label1, RIGHT)
        dir1_label2_grp = [dir1_label2p1, dir1_label2p2, dir1_label2p3]
        


        t = np.linspace(0, 100, 1000) # smoothness of curve later on tied to framerate, not step size
        ...
        dir_arrow = Arrow(
            start = grid.c2p(*-scale*unit(v1_0_dir1)),
            end = grid.c2p(*scale*unit(v1_0_dir1)),
        ).set_opacity(0.75)
        ...
        for i in np.arange(0, 2):
            self.play(
                ReplacementTransform(dir1_label2_grp[i], dir1_label2_grp[i+1])
            )
            self.wait()
        ...
        # add tangent arrow
        self.play(
            Indicate(dir1_label2_grp[2])
        )
        dir_arrow.next_to(dir1_label2_grp[2])
        self.play(
            TransformFromCopy(dir1_label2_grp[2], dir_arrow)
        )
        self.wait(2)
        self.play(
            dir_arrow.animate.move_to(grid.c2p(*v1_0))
        )
        self.wait()
