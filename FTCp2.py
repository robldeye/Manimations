...
class FTCp2(Scene):
    def construct(self):
        ...
        f_label = always_redraw(
            lambda: MathTex(f"f(x)={f(x_value.get_value()):.2f}", color=BLUE).next_to(axes.c2p(1,3)).scale(0.8)
        )
        slope_label = always_redraw(
            lambda: MathTex(r"\text{Slope} =" f"{f(x_value.get_value()):.2f}", color=BLUE).next_to(axes2.c2p(2,3)).scale(0.8)
        )
