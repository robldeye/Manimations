...
class sl3d(ThreeDScene):
    def construct(self):
        ...
        # Mobjects
        a1 = Vector(grid.c2p(1, -4, -3), color=GREEN)
        a2 = Vector(grid.c2p(3, 2, -2), color=YELLOW)
        x1 = ValueTracker(1)
        x1a1 = always_redraw(
            lambda: Vector(grid.c2p(1*x1.get_value(), -4*x1.get_value(), -3*x1.get_value()), color=GREEN)
        )
        span_a2 = Line(
        ...
        sweep = Surface(
            lambda u, v: grid.c2p(u, v, -u + 1/2*v),
            u_range=[-10, 10], 
            v_range=[-10, 10],
        ...
        title = MathTex(r"\text{Span}(\{\vec{a}_1, \vec{a}_2\}) =", r"\left\{ \vec{b} \in \mathbb{R}^3 \mid b_1 - \frac{1}{2}b_2 + b_3 = 0\right\}").to_edge(DOWN)
